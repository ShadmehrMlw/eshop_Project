...
function showLargeImage(imageSrc) {
    ...
    $('#show_large_lightbox').attr('href', imageSrc)
}
...
function removeOrderDetail(detailId) {
    $.get('/user/remove-shop-cart-detail?detail_id=' + detailId).then(res => {
...
function changeOrderDetailCount(detailId, state) {
    $.get('/user/change-shop-cart-detail?detail_id=' + detailId + '&state=' + state).then(res => {
        if (res.status === 'success') {
            $('#order-detail-content').html(res.body);
        }
    });
