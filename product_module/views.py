...
from site_module.models import StieBanner
...
class ProductListView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context['banners'] = StieBanner.objects.filter(is_active= True, postions__iexact=StieBanner.SiteBannerPosition.product_list)
        return context
...
class ProductDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['banners'] = StieBanner.objects.filter(is_active= True, postions__iexact=StieBanner.SiteBannerPosition.product_detail)
        galeries = list(ProductGallery.objects.filter(product_id=loaded_product.id).all())
        galeries.insert(0, loaded_product)
        context['product_galeries_group'] = group_list(galeries, 3)
        context['related_product'] = group_list(list(Product.objects.filter(brand_id=loaded_product.brand.id).exclude(pk=loaded_product.id).all()[:12]),3)
...
def products_brand_components(request):
